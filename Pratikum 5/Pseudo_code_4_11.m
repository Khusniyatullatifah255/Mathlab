...
for i = 1 : N 
    ...
    for j = 3 : M -2 
        ...
        yn(i,M-j) = a2*I(i, (M-j)+1) + a3*I(i, (M-j)+2) - b1*yn(i,(M-j)+1)...
            - b2*yn(i, (M-j)+2);
    end
...
for j = 1 : M
    ...
    yn(N-2,j)= a2 *I(N-2,j) + a3* I (N,j) - b1 *yn(N-1,j) - b2*yn(N,j);
    for i = 3 : N - 2
        yp(i,j) = a0 * I(i,j) + a1*I(i-1,j)- b1*yp(i-1,j) - b2*yp(i-2,j);
        yn(N-i,j) = a2 * I (N-i+1,j) + a3*I(N-i+2,j) - b1*yn(N-i+1,j)...
            - b2*yp(N-i+2,j);
    end
    yp(i,M-1) = a0 * I (i,M-1) + a1*I(i,M-2) - b1*yp(i,M-2) - b2*yp(i,M-3);
    yp(i,M) = a0 * I (i,M) + a1*I(i,M-1) - b1*yn(i,M-1) - b2*yp(i,M-2);
end
...
for j = 1 : M
    ...
    yp(N,j) = c1 * Yx (N-1,j) - b1*yn(N-1,j) - b2*yp(N-2,j);
end
